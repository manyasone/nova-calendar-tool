        ...
        >
                ...
                <div class="p-8">
                    ...
                    <div class="border-b border-40 py-4">
                        ...
                        <date-time-picker
                            ...
                            :first-day-of-week="config.options.firstDay"
                            :date-format="pickerDisplayFormat"
            ...
            <div class="btn-wrapper bg-30 px-6 py-3">
                <button v-if="currentEvent" @click.prevent="handleDelete" type="button"
                        class="btn btn-default btn-danger delete-event">
                ...
                <button @click.prevent="handleSave" ref="saveButton" type="submit"
                        class="btn btn-default btn-primary ml-3">{{ __('Save') }}
                </button>
            </div>
        </form>
    </modal>
...
export default {
    ...
    data() {
        return {
            ...
            start: moment(this.currentEvent !== null ? this.currentEvent.event.start : this.currentDate.date).format(this.dateTimeFormat),
            end: this.currentEvent !== null ? moment(this.currentEvent.event.end).format(this.dateTimeFormat) : moment(this.currentDate.date).add(1, 'hour').format(this.dateTimeFormat)
        }
    },
    methods: {
        changeStart(value) {
    ...
    computed: {
        config() {
            return Nova.config.novaCalendarTool
        },
        pickerDisplayFormat() {
            return Nova.config.novaCalendarTool.pickerDisplayFormat || 'YYYY-MM-DD HH:mm'
        },
        dateTimeFormat() {
            return Nova.config.novaCalendarTool.dateTimeFormat || 'Y-m-d H:i'
        }
    }
}
</script>
